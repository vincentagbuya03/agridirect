...
```mermaid
erDiagram
    %% CORE USER SYSTEM (Top Section)
    USERS ||--o{ USER_ROLES : "has"
    ROLES ||--o{ USER_ROLES : "assigned"
    USERS ||--o| USER_ADDRESSES : "lives at"
    USERS ||--o| USER_SUSPENSIONS : "may have"
    
    %% FARMER SYSTEM (Second Section)
    USERS ||--o| FARMER_PROFILES : "has profile"
    FARMER_PROFILES ||--o{ FARMER_SPECIALIZATIONS : "specializes"
    USERS ||--o| FARMER_REGISTRATIONS : "applies"
    FARMER_REGISTRATIONS ||--o{ FARMER_EDUCATION : "educated"
    FARMER_REGISTRATIONS ||--o{ FARMER_CROP_TYPES : "grows"
    FARMER_REGISTRATIONS ||--o{ FARMER_LIVESTOCK : "raises"
    
    %% PRODUCTS & CATEGORIES (Third Section)
    CATEGORIES ||--o{ PRODUCTS : "contains"
    UNITS ||--o{ PRODUCTS : "measured by"
    USERS ||--o{ PRODUCTS : "creates"
    PRODUCTS ||--o{ PRODUCT_REVIEWS : "has reviews"
    PRODUCT_REVIEWS ||--o{ USERS : "reviewed by"
    PRODUCTS ||--o{ PRODUCT_TAG_MAPPINGS : "tagged"
    PRODUCT_TAGS ||--o{ PRODUCT_TAG_MAPPINGS : "tags"
    
    %% ORDERS (Fourth Section)
    USERS ||--o{ ORDERS : "places"
    USERS ||--o{ ORDERS : "fulfills"
    ORDERS ||--o{ ORDER_ITEMS : "contains"
    ORDER_ITEMS ||--o{ PRODUCTS : "includes"
    
    %% COMMUNITY (Fifth Section)
    USERS ||--o{ FORUM_POSTS : "creates posts"
    FORUM_POSTS ||--o{ FORUM_COMMENTS : "has comments"
    FORUM_COMMENTS ||--o{ USERS : "commented by"
    FORUM_POSTS ||--o{ POST_LIKES : "receives likes"
    POST_LIKES ||--o{ USERS : "liked by"
    
    %% ADMIN & MODERATION (Sixth Section)
    USERS ||--o{ ADMIN_LOGS : "performs"
    USERS ||--o{ REPORTED_CONTENT : "submits reports"
...
### Authentication
- **VERIFICATION_CODES** — Temporary OTP codes for email verification during registration
  - Keyed by `user_email` (not user_id — user does not exist yet at this point)
  - Auto-expires after 10 minutes (`expires_at`)
  - Blocked after 5 failed `attempts` (row deleted)
  - Deleted immediately after successful verification
  - No FK to USERS — pre-registration transient table
...
- **Verification Codes**: UNIQUE(user_email) — One active OTP per email at a time

---

## Data Flow

0. **Email Verification** → VERIFICATION_CODES (OTP generated, emailed via AWS SES, verified, then deleted)
