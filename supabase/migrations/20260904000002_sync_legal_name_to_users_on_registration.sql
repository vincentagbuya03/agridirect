-- Migration: Sync farmer legal name from ID to users table and replace Google name
BEGIN;

-- 1. Backfill all existing farmers' legal names into users table
UPDATE public.users u
SET name = trim(f.full_name),
    updated_at = now()
...
-- 2. Update submit_complete_farmer_registration RPC to automatically sync users.name
DROP FUNCTION IF EXISTS public.submit_complete_farmer_registration(uuid, text, date, text, text, text, text, integer, text, text, text, double precision, double precision, text, text, text, text, jsonb, jsonb, jsonb, boolean, text, double precision, text);
