-- Fix: Allow farmers to re-apply after rejection
-- When a farmer re-applies, reset status to 'pending' and re-activate their account
...
BEGIN
    ...
    -- ON CONFLICT: If user already has a farmer record (e.g. previously rejected),
    -- reset it to pending status and re-activate for new submission
    INSERT INTO farmers (
