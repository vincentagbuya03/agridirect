...
class AdminService extends ChangeNotifier {
  ...
  /// Get all farmer registrations with user info, optionally filtered by status
  Future<List<Map<String, dynamic>>> getFarmerRegistrations({
    String? status, // 'pending' | 'approved' | 'rejected' | null for all
  }) async {
    try {
      // Use SECURITY DEFINER RPC to bypass RLS
      final response = await _client.rpc(
        'get_farmer_registrations_admin',
        params: {'p_status': status},
      );

      if (response == null || response is! List) return [];

      // Transform response to match the expected format with nested 'users' object
      final results = <Map<String, dynamic>>[];
      for (final item in response) {
        final regMap = Map<String, dynamic>.from(item as Map);
        // Map user_name and user_email to nested users object for compatibility
        regMap['users'] = {
          'name': regMap['user_name'] ?? 'Unknown',
          'email': regMap['user_email'] ?? '—',
        };
        results.add(regMap);
      }
      return results;
    } catch (e) {
      _errorMessage = 'Failed to load farmer registrations: $e';
      debugPrint('Error in getFarmerRegistrations: $e');
      notifyListeners();
      return [];
    }
  }
  ...
  Future<bool> approveFarmerRegistration(
    ...
    try {
      await _client.rpc('approve_farmer_registration', params: {
        'p_registration_id': registrationId,
        'p_user_id': userId,
      });

      await _logAdminAction('approve_farmer', 'Approved farmer registration', userId);
      notifyListeners();
      return true;
    } catch (e) {
  ...
  Future<bool> rejectFarmerRegistration(
      String registrationId, String userId, {String? reason}) async {
    try {
      await _client.rpc('reject_farmer_registration', params: {
        'p_registration_id': registrationId,
      });

      await _logAdminAction(
          'reject_farmer', reason ?? 'Rejected farmer registration', userId);
      notifyListeners();
      return true;
    } catch (e) {
