...
class _ConsumerRadarViewState extends State<ConsumerRadarView> {
  ...
  void initState() {
    super.initState();
    _viewType = 'consumer-windy-iframe-${DateTime.now().millisecondsSinceEpoch}';

    ui_web.platformViewRegistry.registerViewFactory(
      _viewType,
      (int viewId) {
        final iframe = html.IFrameElement()
          ..src = 'https://embed.windy.com/embed2.html?lat=${widget.lat}&lon=${widget.lon}&detailLat=${widget.lat}&detailLon=${widget.lon}&zoom=10&level=surface&overlay=rain&product=ecmwf&menu=&message=&marker=true&calendar=now&pressure=&type=map&location=coordinates&metricWind=km%2Fh&metricTemp=%C2%B0C&radarRange=-1'
          ..style.border = 'none'
          ..style.height = '100%'
          ..style.width = '100%'
          ..style.display = 'block'
